...
whole thing keeps working on a plane.
    ...
    scripts/generate_tts.py --phrases a b c       ->  voice/phrase/general/a.mp3
    scripts/generate_tts.py --phrases

    # the same letters, but for a different set of words
    scripts/generate_tts.py --phrases --set vehicles  ->  voice/phrase/vehicles/a.mp3
...
of the letter A would both want to be a.mp3. Phrases collide across word sets for
the same reason -- "A is for apple" and "A is for ambulance" are both A -- so each
set gets its own directory. Recording phrases also rewrites src/lib/voice-clips.ts,
which is how the app knows which letters can speak.
...
def write_manifest() -> None:
    ...
    Every word set is rescanned, not just the one that was recorded, so the
    manifest always describes the whole phrase directory. A generated module
    rather than a manifest the app fetches: it is build-time knowledge, so it
    can be type-checked and costs nothing at runtime.
    """
    entries = []
    total = 0
    for name in word_sets():
        have = sorted(p.stem for p in (PHRASE_DIR / name).glob("*.mp3"))
        total += len(have)
        entries.append(f"\t{name}: [" + ", ".join(f"'{c}'" for c in have) + "]")
    MANIFEST.write_text(
    ...
    print(f"{shown(MANIFEST)}: {total} clip(s) across {len(entries)} set(s)")
