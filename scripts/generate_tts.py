...
whole thing keeps working on a plane.
    ...
    # every letter, word and number the app uses (62 clips)
    scripts/generate_tts.py --all

    # see what --all would record, without calling out
    scripts/generate_tts.py --all --dry-run
...
DEFAULT_OUT = ROOT / "static" / "sounds" / "voice"
...
def main() -> None:
    ...
    if not args.all and not args.phrase:
        ap.error("give a phrase to record, or --all")
