...
whole thing keeps working on a plane.
    ...
    scripts/generate_tts.py --phrases a b c       ->  voice/phrase/a-apple.mp3
    scripts/generate_tts.py --phrases

    # the same letters, but for a different set of words
    scripts/generate_tts.py --phrases --set vehicles  ->  voice/phrase/a-ambulance.mp3
...
The two kinds are kept apart because they collide: the phrase for A and the name
of the letter A would both want to be a.mp3. A phrase is named for its letter and
its word together -- a-apple, a-ambulance -- which keeps the word sets from
colliding in the same way, and lets them share what they have in common: nest is
N in both sets, so n-nest.mp3 is recorded once and played by both. Recording
phrases also rewrites src/lib/voice-clips.ts, which is how the app knows which
lines can be spoken.
...
def word_sets() -> list[str]:
    """The word set names the app offers, in the order it offers them."""
    src = WORDS_TS.read_text()
    found = re.search(r"WORD_SET_KEYS: WordSet\[\] = \[(.*?)\]", src, re.S)
    if not found:
        sys.exit(f"could not find WORD_SET_KEYS in {WORDS_TS}; has its shape changed?")
    return re.findall(r"'([^']+)'", found.group(1))


def load_words(word_set: str = "general") -> tuple[dict[str, str], list[str]]:
    """Read one word set and NUMBER_WORDS out of the app's own source.
    ...
    const = word_set.upper()
    body = re.search(rf"const {const}: Record<string, string> = \{{(.*?)\}}", src, re.S)
    numbers = re.search(r"NUMBER_WORDS = \[(.*?)\]", src, re.S)
    if not body:
        sys.exit(f"no word set {word_set!r} in {WORDS_TS}; try one of {', '.join(word_sets())}")
    if not numbers:
        sys.exit(f"could not find NUMBER_WORDS in {WORDS_TS}; has its shape changed?")
...
def phrase_line_list(letters: list[str] | None, word_set: str) -> list[tuple[str, str]]:
    """"A is for apple" for each letter, as (name, text) pairs.
...
def write_manifest() -> None:
    """Tell the app which lines can be spoken, from what is actually on disk.

    One flat list of clip names covering every word set, because that is what the
    directory is. A generated module rather than a manifest the app fetches: it
    is build-time knowledge, so it can be type-checked and costs nothing at
    runtime.
    """
    ...
    print(f"{shown(MANIFEST)}: {len(have)} line(s) can be spoken")
...
def full_line_list() -> list[tuple[str, str]]:
    ...
    a digit is spoken as its number word, so 3 and 'three' do too. Every word
    set contributes its words, and a word in two sets is recorded once.
    """
    every: dict[str, str] = {}
    for name in word_sets():
        every.update(load_words(name)[0])
    _, number_words = load_words()
    lines = [(letter, LETTER_TEXT[letter]) for letter in sorted(LETTER_TEXT)]
    lines += [(slug(word), word) for word in sorted(set(every.values()))]
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default="general",
        metavar="NAME",
        help="which word set --phrases records (default: general)",
    )
    ...
    if args.phrases is not None:
        if args.set not in word_sets():
            ap.error(f"unknown word set {args.set!r}: pick from {', '.join(word_sets())}")
        lines += phrase_line_list(args.phrases or None, args.set)
